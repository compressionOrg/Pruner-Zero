...
def prune_DSnoT_dlp(
    ...
    """
    DSnoT 的按层稀疏率（DLP）版本。
    先基于层重要性计算每层稀疏率，再复用 DSnoT 的逐层剪枝逻辑。
    """
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    if torch.isclose(max_ratio, min_ratio):
        ...
        scaled_ratios = (imp_ratios - min_ratio) * (
            1 / (max_ratio - min_ratio) * args.alpha * 2
        )
        all_layer_ratio = (
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            print(f"pruning layer {i} name {name} with layer sparsity {layer_sparsity_ratio:.4f}")
            start_time = time.time()
            ...
            if prune_n != 0:
                ...
                sparsity_num = int(initial_metric.shape[1] * layer_sparsity_ratio)
                res_sparsity_num = sorted_initial_indice.shape[1] - sparsity_num
...
def prune_DSnoT_dlp_auto(
    args, model_loader, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0
):
    """
    带自动alpha确定功能的DSnoT DLP剪枝方法。

    Args:
        args: 包含配置参数的对象
        model_loader: 模型加载函数，用于重新加载全新模型以节省显存
        tokenizer: 分词器
        device: 计算设备
        prune_n: 结构化剪枝参数n
        prune_m: 结构化剪枝参数m
    """
    from temp.auto_alpha import ternary_search_alpha
    import gc

    print("开始自动alpha确定的DSnoT DLP剪枝...")

    if hasattr(args, "auto_alpha") and args.auto_alpha:
        print("启用自动alpha确定功能 (节省显存模式)")

        alpha_min = getattr(args, "alpha_min", 0.01)
        alpha_max = getattr(args, "alpha_max", 0.2)
        ...
        prune_DSnoT_dlp(args, model, tokenizer, device, prune_n, prune_m)
